...
@section('content')
    <div class="box-container">
        <div class="page-title mt-7">
            <h1 class="text-2xl font-semibold">Новости</h1>
        ...
        <div class="list-block" id="list-block">
            ...
            <div class="table-header flex w-full h-11 items-center bg-blue-600 rounded mt-6">
                ...
                <p class="table-header-addr w-56 ml-4 text-center text-sm text-white">Короткое название</p>
                <p class="table-header-user-name w-44 ml-4 text-center text-sm text-white">Дата создания</p>
            </div>
                ...
                @if($news)
                    @foreach($news as $news_item)
                        <li class="mt-2 relative flex items-center border rounded-md border-gray-300 py-4">
                            <p class="num w-12 ml-4 text-center text-sm">{{$news_item['id']}}</p>
                            <p class="name w-48 ml-4 text-center text-sm">{{$news_item['news_title']}}</p>
                            <p class="addr w-56 ml-4 text-center text-sm">{{$news_item['news_short']}}</p>
                            <p class="user-name w-44 ml-4 text-center text-sm">{{$news_item['date_create']}}</p>
